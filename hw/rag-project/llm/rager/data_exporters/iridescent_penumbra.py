...
def elasticsearch(
    ...
    *args, **kwargs,
    ...
    set_global_variable('intergalactic_solaris', 'index_name', index_name)
    ...
    print(f'Received {len(documents)} documents for indexing.')
    if len(documents) > 0:
        print("Sample document:", documents[0])
    
    print(f'Indexing {len(documents)} documents to Elasticsearch index {index_name}')
    for document in documents:
        print(f'Indexing document {document.get("document_id", "unknown")}')
